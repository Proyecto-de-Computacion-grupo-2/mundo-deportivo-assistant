...
def scrape_personal_lineup_fantasy(app: bool, driver, user):
    if not app:
        driver = helper.login_fantasy_mundo_deportivo()
    ...
    if driver is not None:
        ...
        if app:
            with open(helper.path.join(route.users_folder, user + "_" + route.app_personal_lineup_file), "w",
                      newline = "") as csv_file:
...
if __name__ == "__main__":
    ...
    scrape_personal_lineup_fantasy(False, None, None)
    helper.extract()
